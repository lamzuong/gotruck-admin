...
import {
  ...
  HeaderTable,
  HeaderTableHistory,
} from '../MyTableFormWithdraw/MyTableFormWithdraw';
...
import { faArrowLeftLong, faCircleXmark, faSearch } from '@fortawesome/free-solid-svg-icons';
...
function HistoryWithdraw() {
  ...
  useEffect(() => {
    ...
    const fetchApi = async () => {
      const res = await formWithDrawAPI.searchNoPageHistory({
        id_transaction_history: debouncedShipperId.toUpperCase(),
      });
      setTotalItems(res);

      const result = await formWithDrawAPI.searchHistory({
        page: page,
        limit: 10,
        id_transaction_history: debouncedShipperId.toUpperCase(),
      });
  ...
  useEffect(() => {
    const getFormWithdraw = async () => {
      try {
        const total = await formWithDrawAPI.getByNoPageHistory();
        setTotalItems(total);
        const res = await formWithDrawAPI.getByPageHistory({
          limit: 10,
          page: page,
        });
        setListWithDraw([...res]);
    ...
    <div className={cx('wrapper')}>
      ...
      <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
        ...
        <h1 style={{ fontWeight: 'bold', fontSize: 26, marginLeft: 15 }}>
          Lịch sử xử lý đơn yêu cầu rút tiền
        </h1>
      </div>
      <div
